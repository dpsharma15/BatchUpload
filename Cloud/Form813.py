...
def app():
    # Streamlit app setup
    st.markdown("<h1 style='text-align: center; width: 100%;'>Form-813 File Uploader/Downloader</h1>", unsafe_allow_html=True)
    ...
    if st.button("Execute Program"):
        ...
        if file_scdb_path and file_subcontractor_path and file_scdb_mapping_path and file_template_path:
            try:
                ...
                df_master.columns=['Task ID', 'Task Model (Name)', 'Tag No.', 'Task Status']
                ...
                # create_download_button(excel_file, "Download Report", "Report.xlsx")
                ...
                for index, _ in result_df.iterrows():
                    ...
                    Temp_update['Completed Date'] = result_df.loc[index, 'Submitted Date']
                    ...
                    Temp_update.at[1,'Parameter Reading 01']=result_df.at[index,'From']
                    Temp_update.at[1,'Parameter Reading 02']=result_df.at[index,'To']
                    Temp_update.at[1,'Step Answer']=result_df.at[index,'1. Cable installed as per trench/Tray cross section details and drawing']
                    Temp_update.at[2,'Step Answer']=result_df.at[index,'2. Check cable routing as per layout drawing. And the bending radius as per manufacture recommendation']
                    Temp_update.at[3,'Step Answer']=result_df.at[index,'3. Cable dressing segregation and spacing (different voltage level, instrument, PAGA, telecom cables) are according to approved drawing and no damage to cable outer insulation']
                    Temp_update.at[4,'Step Answer']=result_df.at[index,'4. Cable identification tags provided at specified intervals, route/direction chnages and at each bend as per spproved drawings.']
                    Temp_update.at[5,'Step Answer']=result_df.at[index,'5. Chek cable identification tags atre provided at both ends of duct bank (where Applicable)']
                    Temp_update.at[6,'Step Answer']=result_df.at[index,'6. Single core cables are laid in trfoil formation (where applicable)']
                    Temp_update.at[7,'Step Answer']=result_df.at[index,'7. Sealing of duct bank including spare with approved sealent after cable installation']
                    Temp_update.at[8,'Step Answer']=result_df.at[index,'8. Cable trench clean and free from stoned and foreign debris']
            ...
            # Save DataFrame to BytesIO object
                ...
                for row_num, row in enumerate(sheet.iter_rows(min_row=2, max_row=sheet.max_row), start=2):
                    completed_date_cell = sheet.cell(row=row_num, column=column_indices['Completed Date'])  # Get the 'Completed Date' cell
                    ...
                    completed_by_cell = sheet.cell(row=row_num, column=column_indices['Completed By'])  # Get the 'Completed By' cell
